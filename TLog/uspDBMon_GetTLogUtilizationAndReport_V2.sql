	...
	BEGIN
		--Database(s) exceed the threshold. Need to Alert!.					
			...
			SET @tableHTML =
								...
								<H2>Monitor Transaction Log</H2>

								<table>
								<tr>
									<th>SQL Server Instance</th>
									<th>Database Name</th>
									<th>Log Size(MB)</th>
									<th>Log Space Used(%)</th>
									<th>Log Reuse Wait Desc</th>
...
PRINT 2										
            SELECT @tableHTML +=
            N'<tr>
                <td>' + CAST(SERVERPROPERTY('servername') AS SYSNAME) + N'</td>
                <td>' + [DB_Name] + N'</td>
				<td>' + CAST([Log_Size_MB] AS NVARCHAR(1000)) + N'</td>
				<td style="font-weight:bold;color:red;">' + CAST([Log_Space_Used%] AS NVARCHAR(1000)) + N'</td>
				<td>' + [log_reuse_wait_desc] + N'</td>
...
PRINT 5
				IF (@Mail_Flag <> 0)
					BEGIN
						SELECT @mailSubject = '[TLog Space Utilization Report]: ' + CAST(SERVERPROPERTY('servername') as varchar(255)) + ' Transaction Log utilization exceeds threshold of ' + CAST(@TLog_Usage_Threshold AS VARCHAR(10)) + '%'
